import { PrismaClient, ProcessType, ShipmentStatus } from "@prisma/client";
...
async function main() {
  ...
  const processes: ProcessType[] = ["WASHED", "NATURAL", "HONEY", "EXPERIMENTAL"];
  ...
  for (let year of [2023, 2024]) {
    for (let i = 1; i <= 18; i++) {
      ...
      const statuses: ShipmentStatus[] = ["SAMPLE", "MILLED", "READY", "IN_TRANSIT", "LANDED"];
